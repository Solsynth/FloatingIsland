    ...
    <div class="mx-auto max-w-4xl">
      ...
      <template v-else>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 mb-6">
          <div v-for="stat in statsCards" :key="stat.label" class="card bg-base-100 shadow-sm">
            ...
            <div class="flex items-center justify-between">
              <div>
                <div class="flex items-center gap-2 mb-2">
                  <component :is="ratingIcon" class="w-6 h-6" :class="ratingTextColor" />
                  <span class="text-2xl font-bold" :class="ratingTextColor">{{ rating.grade }}</span>
                </div>
                <p class="text-sm opacity-80" :class="ratingTextColor">
                  {{ t('creator.rating.title') }}: {{ rating.rating.toFixed(1) }}
                </p>
              </div>
              <div class="text-right">
                <div class="text-4xl font-bold" :class="ratingTextColor">{{ formatRating(rating.rating) }}</div>
                <div class="text-sm opacity-80" :class="ratingTextColor">{{ t('creator.rating.rank', { rank: rating.rank }) }}</div>
                <div class="text-xs opacity-60" :class="ratingTextColor">{{ t('creator.rating.percentile', { percentile: rating.percentile.toFixed(1) }) }}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="card bg-base-100 shadow-sm">
          <div class="card-body p-2">
            <div class="divide-y divide-base-200">
              ...
              >
                <component :is="item.icon" class="w-5 h-5 text-base-content/60" />
    ...
    <template #rightbar>
      <div class="space-y-4 card bg-base-100 shadow-sm rounded-none min-h-full p-4">
            ...
            <div class="flex items-center gap-3 mb-3">
              <div class="avatar">
                <div class="w-10 rounded-full">
                  <img v-if="getFileUrl(currentPublisher.picture?.id)" :src="getFileUrl(currentPublisher.picture?.id)" :alt="currentPublisher.nick" />
              ...
              <div class="min-w-0">
                <div class="font-semibold text-sm truncate">{{ currentPublisher.nick }}</div>
                <div class="text-xs text-base-content/50">@{{ currentPublisher.name }}</div>
              </div>
            </div>
            <p v-if="currentPublisher.bio" class="text-xs text-base-content/60 line-clamp-3">
        ...
        <div v-if="stats" class="card bg-base-100 shadow-sm">
          <div class="card-body p-4">
            <h3 class="font-semibold text-sm mb-3">{{ t('creator.dashboard') }}</h3>
            <div class="space-y-2">
              <div class="flex items-center justify-between text-sm">
                <span class="text-base-content/60">{{ t('creator.stats.postsCreated') }}</span>
                <span class="font-medium">{{ stats.postsCreated }}</span>
              </div>
              <div class="flex items-center justify-between text-sm">
                <span class="text-base-content/60">{{ t('creator.stats.upvoteReceived') }}</span>
                <span class="font-medium">{{ stats.upvoteReceived }}</span>
              </div>
              <div class="flex items-center justify-between text-sm">
                <span class="text-base-content/60">{{ t('creator.stats.downvoteReceived') }}</span>
        ...
        <div v-if="rating" class="card bg-base-100 shadow-sm">
          <div class="card-body p-4">
            <h3 class="font-semibold text-sm mb-3">{{ t('creator.rating.title') }}</h3>
            <div class="flex items-center gap-3">
              <span class="text-3xl font-bold" :class="ratingTextColor">{{ rating.grade }}</span>
              <div>
                <div class="text-sm font-medium">{{ formatRating(rating.rating) }}</div>
                <div class="text-xs text-base-content/50">{{ t('creator.rating.rank', { rank: rating.rank }) }}</div>
              </div>
            </div>
          </div>
        </div>
...
defineOgImage('UniOgImage', { title: computed(() => `${currentPublisher.value?.nick ?? pubName.value} - ${t('creator.dashboard')}`) })
...
onMounted(async () => {
  if (!currentPublisher.value || currentPublisher.value.name !== pubName.value) {
...
const statsCards = computed(() => {
  ...
  return [
    ...
    { value: s.stickerPacksCreated, label: t('creator.stats.stickerPacksCreated') },
