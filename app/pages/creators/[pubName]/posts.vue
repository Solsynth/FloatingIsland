    ...
    <div class="mx-auto max-w-4xl">
      ...
      <div v-else class="space-y-2">
            ...
            <div class="flex items-start gap-3">
              ...
              <div class="min-w-0 flex-1">
                ...
                <div class="flex items-center gap-3 mt-2 text-xs text-base-content/50">
                  ...
                  <span class="flex items-center gap-1">
                    <IconMessageCircle class="w-3 h-3" /> {{ post.repliesCount }}
                  </span>
                  <span class="ml-auto">{{ formatDate(post.publishedAt || post.createdAt) }}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div v-if="hasMore" class="flex justify-center py-4">
          <button class="btn btn-ghost btn-sm" :class="{ 'loading': loadingMore }" @click="loadMore">
            {{ t('common.loadMore') }}
          </button>
        </div>
      </div>
...
defineOgImage('UniOgImage', { title: computed(() => `${t('creator.posts.title')} - ${currentPublisher.value?.nick ?? pubName.value}`) })
...
async function loadMore() {
  ...
  try {
    const result = await fetchPublisherPosts(pubName.value, pageSize, offset.value)
