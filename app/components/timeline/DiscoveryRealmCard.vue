  ...
  <div class="card bg-base-100 shadow-sm">
    <div class="card-body p-4">
      <div class="flex items-center gap-2 mb-3">
        <IconGlobe class="h-4 w-4 text-primary" />
        <span class="text-xs font-medium text-base-content/60">Suggested Realm</span>
      </div>
      <div class="flex items-center gap-3">
        <div v-if="realmPictureUrl" class="avatar">
          <div class="h-12 w-12 rounded-lg">
            <img :src="realmPictureUrl" :alt="realm.name" class="h-full w-full rounded-lg object-cover" />
          </div>
        </div>
        ...
        <div class="flex-1 min-w-0">
          ...
          <p v-if="realm.description" class="text-xs text-base-content/60 line-clamp-2 mt-0.5">{{ realm.description }}</p>
        </div>
      </div>
      <div class="flex items-center gap-2 mt-3">
        <NuxtLink :to="`/realms/${realm.slug}`" class="btn btn-primary btn-xs flex-1">
          View Realm
        </NuxtLink>
      </div>
