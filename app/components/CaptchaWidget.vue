<template>
    <div class="flex w-full flex-col items-center gap-3">
        ...
        >
            <IconAlertTriangle class="w-[18px] h-[18px]" />
            <span>{{ error }}</span>
        </div>
        <div
            ref="captchaContainer"
            class="flex min-h-[70px] w-full justify-center"
        ></div>
    </div>
</template>
...
async function renderWidget() {
    if (typeof window === 'undefined' || typeof document === 'undefined') return;
    if (!captchaContainer.value || !currentProvider.value || !currentApiKey.value) return;
    ...
    try {
        ...
        error.value = err instanceof Error ? err.message : 'Failed to initialize captcha';
    } finally {
