...
const { publisherName } = defineProps<{
...
// Fetch publisher data server-side
const { data: publisher } = await useAsyncData(
  `og-publisher-${publisherName}`,
  () => $fetch(`${apiBaseUrl}/sphere/publishers/${encodeURIComponent(publisherName)}`).then(snakeToCamel),
  { server: true }
)

// Computed values
const name = computed(() => publisher.value?.name || publisherName)
const displayName = computed(() => publisher.value?.nick || publisher.value?.name || publisherName)
const bio = computed(() => publisher.value?.bio || '')
const avatarId = computed(() => publisher.value?.picture?.id || '')
const backgroundId = computed(() => publisher.value?.background?.id || '')
