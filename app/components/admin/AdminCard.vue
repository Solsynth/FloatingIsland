<template>
  <div
    ...
    :class="[hoverable ? 'transition-all hover:shadow-md hover:border-base-300/60 cursor-pointer' : '']"
  >
    <!-- Header -->
    <div
    ...
    >
      <div v-if="title" class="min-w-0">
        ...
        <p v-if="description" class="text-xs text-base-content/40 mt-0.5">{{ description }}</p>
      </div>
      <slot name="header" />
    </div>

    <!-- Body -->
    <div :class="[compact ? 'p-4' : 'p-5', noPadding ? 'p-0' : '']">
      <slot />
    </div>
