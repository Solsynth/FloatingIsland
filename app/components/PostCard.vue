  ...
  >
    <div class="card-body p-4 sm:p-5">
      ...
      <div class="flex items-center gap-3 mb-2">
        <div class="avatar">
          <div class="w-10 h-10 rounded-full bg-base-300 overflow-hidden">
            ...
            <div v-else class="w-full h-full flex items-center justify-center text-base-content/50">
              <span class="text-sm font-medium">{{ publisherInitials }}</span>
            </div>
          </div>
        </div>
        <div class="flex-1 min-w-0">
          <div class="flex items-center gap-1">
            <span class="font-semibold truncate">{{ publisherName }}</span>
            <IconBadgeCheck v-if="isVerified" class="w-4 h-4 text-blue-500 flex-shrink-0" />
          </div>
        ...
        <!-- Visibility Badge -->
        <div class="badge badge-sm" :class="visibilityClass" :title="visibilityLabel">
      ...
      <!-- Content -->
      <div class="mt-2">
        <h3 v-if="post.title" class="font-bold text-lg mb-1">{{ post.title }}</h3>
        <p v-if="post.description" class="text-sm text-base-content/70 mb-2">{{ post.description }}</p>
        <div
          class="text-base whitespace-pre-wrap break-words"
          :class="{ 'line-clamp-4': isExpanded === false && post.isTruncated }"
      ...
      <div v-if="hasEmbeds" class="mt-3">
        ...
        >
          <template v-if="embed?.type === 'link'">
            <a :href="embed.url" class="text-primary hover:underline" @click.stop>
      ...
      <div class="flex items-center gap-4 mt-3 text-sm text-base-content/50">
        <span v-if="post.viewsUnique > 0" class="flex items-center gap-1">
          <IconEye class="w-3.5 h-3.5" />
          {{ formatNumber(post.viewsUnique) }}
        </span>
        <span v-if="post.repliesCount > 0" class="flex items-center gap-1">
          <IconMessageCircle class="w-3.5 h-3.5" />
          {{ formatNumber(post.repliesCount) }}
        </span>
        <span v-if="post.boostCount > 0" class="flex items-center gap-1">
...
const publisherName = computed(() => props.post.publisher?.nick || props.post.publisher?.name || 'Unknown')
...
// Visibility
const VISIBILITY_CONFIG: Record<number, { label: string; icon: string; class: string }> = {
