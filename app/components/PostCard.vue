<template>
    <component :is="wrapperTag" class="block hover:no-underline" v-bind="wrapperProps">
            ...
            <div class="card-body p-4">
                ...
                <div v-if="showReference && hasReference" class="mb-2">
                    ...
                    >
                        ...
                        <IconChevronDown v-if="referenceCollapsed" class="h-4 w-4 text-base-content/60" />
                    ...
                    >
                        <div class="relative flex justify-center">
                            <button
                                type="button"
                                class="avatar"
                                @click.stop="navigateToPublisher(referencePost.publisher.name)"
                            >
                                <div v-if="getAvatarUrl(referencePost)" class="h-10 w-10 rounded-full">
                                    <img
                                ...
                                <div v-else class="h-10 w-10 rounded-full bg-primary text-primary-content">
                                    <span class="text-sm font-medium">{{ getInitials(getDisplayName(referencePost)) }}</span>
                                </div>
                            </button>
                        ...
                        <div class="min-w-0 pb-2">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="truncate text-sm text-base-content/65">@{{ referencePost.publisher.name }}</span>
                            </div>
                            <div class="text-xs text-base-content/55">
                                {{ formatDate(referencePost.publishedAt) }}
                            </div>
                            <div v-if="referencePost.content" class="prose prose-sm mt-2 max-w-none text-base-content/85" v-html="renderedReferenceContent"></div>
                            <div v-if="referencePost.title || referencePost.description" class="mt-2 space-y-1">
                                <div v-if="referencePost.title" class="line-clamp-1 text-sm font-semibold">{{ referencePost.title }}</div>
                                <div v-if="referencePost.description" class="line-clamp-2 text-xs text-base-content/75">{{ referencePost.description }}</div>
                            </div>
                            <div
                            ...
                            <div v-if="referencePost.attachments.length > 0" class="mt-2 inline-flex items-center gap-1 text-xs text-base-content/70">
                                <IconPaperclip class="h-3.5 w-3.5" />
                                <span>{{ referencePost.attachments.length }} attachment(s)</span>
                            </div>
                            <AttachmentGrid v-if="referencePost.attachments.length > 0" :attachments="referencePost.attachments" :max-height="80" />
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-start justify-between gap-3">
                    <button
                        ...
                        @click.stop="navigateToPublisher(post.publisher.name)"
                    >
                        <!-- Avatar -->
                        ...
                        <div class="flex flex-col items-start">
                            <div class="flex items-center gap-1.5">
                                ...
                                <IconBadgeCheck v-if="post.publisher.verification" class="h-4 w-4 text-primary" />
                                <span class="text-sm text-base-content/50">@{{ post.publisher.name }}</span>
                            </div>
                            <span class="text-xs text-base-content/40">{{ formatDate(post.publishedAt, isDetail) }}</span>
                        </div>
                    </button>
                ...
                <!-- Content -->
                <div class="mt-3">
                    <h3 v-if="post.title" class="mb-2 text-lg font-semibold">{{ post.title }}</h3>
                    <div class="prose-xs prose max-w-none text-base-content/90" v-html="renderedContent"></div>
                    <div
                ...
                <!-- Attachments -->
                <AttachmentGrid v-if="post.attachments.length > 0" :attachments="post.attachments" />
                ...
                <div v-if="embeds.length > 0" class="mt-3 space-y-3">
                    <!-- Link Embeds -->
                    <div v-if="linkEmbeds.length > 0" class="rounded-xl border border-base-300/80 bg-base-200/20 p-2">
                        ...
                        <div v-if="linkEmbeds.length === 1">
                            ...
                            >
                                <img
                                    v-if="getEmbedImage(linkEmbeds[0]) && getEmbedImage(linkEmbeds[0]) !== getEmbedFavicon(linkEmbeds[0])"
                                    :src="resolveAssetUrl(getEmbedUrl(linkEmbeds[0])!, getEmbedImage(linkEmbeds[0])!)"
                                ...
                                <div class="card-body gap-2 p-3">
                                    <div class="flex items-center gap-2 text-xs text-base-content/60">
                                        <img
                                            ...
                                            :src="resolveAssetUrl(getEmbedUrl(linkEmbeds[0])!, getEmbedFavicon(linkEmbeds[0])!)"
                                            alt="Site icon"
                                            class="h-4 w-4 rounded object-cover"
                                            loading="lazy"
                                        />
                                        <IconLink v-else class="h-4 w-4" />
                                        <span class="truncate">{{ getEmbedSiteName(linkEmbeds[0]) || getHost(getEmbedUrl(linkEmbeds[0])!) }}</span>
                                        <IconExternalLink class="ml-auto h-4 w-4" />
                                    </div>
                                    <div v-if="getEmbedTitle(linkEmbeds[0])" class="line-clamp-2 text-sm font-semibold">{{ getEmbedTitle(linkEmbeds[0]) }}</div>
                                    <div v-if="getEmbedDescription(linkEmbeds[0])" class="line-clamp-3 text-sm text-base-content/75">{{ getEmbedDescription(linkEmbeds[0]) }}</div>
                                    <div class="truncate text-xs text-primary underline">{{ getEmbedUrl(linkEmbeds[0]) }}</div>
                                    <div v-if="getEmbedAuthor(linkEmbeds[0]) || getEmbedPublished(linkEmbeds[0])" class="mt-1 flex items-center gap-3 text-xs text-base-content/60">
                                        <span v-if="getEmbedAuthor(linkEmbeds[0])" class="inline-flex items-center gap-1">
                                            <IconUser class="h-3.5 w-3.5" />
                                            {{ getEmbedAuthor(linkEmbeds[0]) }}
                                        </span>
                                        <span v-if="getEmbedPublished(linkEmbeds[0])" class="inline-flex items-center gap-1">
                        ...
                        <div v-else class="carousel w-full carousel-center gap-3">
                            ...
                            >
                                ...
                                <div class="card-body p-3">
                                    <div class="truncate text-xs text-base-content/60">{{ getEmbedSiteName(embed) || getHost(getEmbedUrl(embed)!) }}</div>
                                    <div class="line-clamp-1 text-sm font-semibold">{{ getEmbedTitle(embed) || getEmbedUrl(embed) }}</div>
                                    <div v-if="getEmbedDescription(embed)" class="line-clamp-2 text-xs text-base-content/70">{{ getEmbedDescription(embed) }}</div>
                                </div>
                            </button>
                        </div>
                    </div>
                    ...
                    <div v-for="(embed, idx) in nonLinkEmbeds" :key="idx">
                        ...
                        <div v-if="getEmbedType(embed) === 'poll'" class="card">
                            <div class="card-body p-3">
                                ...
                                <div v-if="!getEmbedId(embed)" class="text-sm text-base-content/70">Poll was unavailable.</div>
                                <div v-else>
                                    <div class="text-sm text-base-content/80">{{ getEmbedTitle(embed) || `Poll #${getEmbedId(embed)}` }}</div>
                                    <div v-if="getEmbedDescription(embed)" class="text-xs text-base-content/65">{{ getEmbedDescription(embed) }}</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div v-else-if="getEmbedType(embed) === 'fund'" class="card">
                            <div class="card-body p-3">
                                ...
                                <div v-if="!getEmbedId(embed)" class="text-sm text-base-content/70">Fund envelope was unavailable.</div>
                                <div v-else>
                                    <div class="text-sm text-base-content/80">{{ getEmbedTitle(embed) || `Fund #${getEmbedId(embed)}` }}</div>
                                    <div v-if="getEmbedDescription(embed)" class="text-xs text-base-content/65">{{ getEmbedDescription(embed) }}</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div v-else-if="getEmbedType(embed) === 'livestream'">
                            ...
                            >
                                <div class="card-body p-3">
                                    ...
                                    <div v-if="getEmbedDescription(embed)" class="text-sm text-base-content/70 line-clamp-2">{{ getEmbedDescription(embed) }}</div>
                                </div>
                            </NuxtLink>
                        </div>
                        ...
                        <div v-else class="alert py-2 text-sm alert-info">
                            ...
                            <span>Unable to show embed: {{ getEmbedType(embed) || 'unknown' }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-3 flex flex-wrap items-center gap-2 text-xs">
                    <span v-if="post.visibility !== 0" :class="`badge badge-sm ${visibilityMeta.tone}`">
                    ...
                    <span v-if="post.attachments.length > 0" class="badge badge-outline badge-sm">
...
// Computed
const avatarUrl = computed(() => getFileUrl(props.post.publisher.picture?.id));
const displayContent = computed(() => props.post.isTruncated ? `${props.post.content}...` : props.post.content);
const renderedContent = computed(() => renderMarkdown(displayContent.value));
const referencePost = computed(() => props.post.repliedPost ?? props.post.forwardedPost);
...
const renderedReferenceContent = computed(() => {
    ...
    const content = referencePost.value.isTruncated ? `${referencePost.value.content}...` : referencePost.value.content;
...
const linkEmbeds = computed(() => embeds.value.filter((e) => getEmbedType(e) === 'link'));
const nonLinkEmbeds = computed(() => embeds.value.filter((e) => getEmbedType(e) !== 'link'));
...
function getDisplayName(target: Post['publisher']): string {
    return target.nick || target.name;
}
